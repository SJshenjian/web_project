...
import com.haotu369.jvm.cmd.CommandParser;
...
public class CodeAttr extends AttributeInfo {
    ...
    public static AttributeInfo parseCodeAttr(ClassFile clzFile,
        ...
        for (int i = 1; i <= attrCount; i++) {
            ...
            if (LINE_NUMBER_TABLE.equalsIgnoreCase(subAttrName)) {

                AttributeInfo table = LineNumberTable.parse(iter);
                codeAttr.setLineNumberTable(table);

            } else if (LOCAL_VARIABLE_TABLE.equalsIgnoreCase(subAttrName)) {

                AttributeInfo table = LocalVariableTable.parse(iter);
