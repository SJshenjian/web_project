...
import online.shenjian.tio.im.common.http.HttpRequestDecoder;
import online.shenjian.tio.im.common.http.HttpRequestPacket;
import online.shenjian.tio.im.common.http.HttpResponseEncoder;
import online.shenjian.tio.im.common.http.HttpResponsePacket;
import online.shenjian.tio.im.common.http.websocket.WebSocketDecoder;
import online.shenjian.tio.im.common.http.websocket.WebSocketEncoder;
import online.shenjian.tio.im.common.http.websocket.WebSocketPacket;
...
import com.shenjian.online.tio.im.server.server.handler.*;
import online.shenjian.online.tio.im.server.server.handler.*;
import online.shenjian.tio.im.server.server.handler.*;
import online.shenjian.tio.im.server.service.UserService;
import online.shenjian.tio.im.common.packets.ChatRespBody;
import online.shenjian.tio.im.common.packets.ChatType;
...
public class ImAioServerHandler implements ServerAioHandler {
    ...
    public Packet decode(ByteBuffer buffer, int limit, int position, int readableLength, ChannelContext channelContext) throws AioDecodeException {
        ...
        if (isWebSocket) {
            ...
            WebSocketPacket.Opcode opcode = webSocketPacket.getOpcode();
            if (WebSocketPacket.Opcode.BINARY == opcode) {
            ...
            if (WebSocketPacket.Opcode.PING == opcode || WebSocketPacket.Opcode.PONG == opcode) {
                return heartbeatPacket;
            }
            if (WebSocketPacket.Opcode.CLOSE == opcode) {
                WebSocketPacket imPacket = new WebSocketPacket(Command.COMMAND_CLOSE_REQ);
                return imPacket;
            }
            if (WebSocketPacket.Opcode.TEXT == opcode) {
