	...
	<div id="wrap">
		...
		<div class="tabs_header">
			<ul id="ul_tab" class="tabs" style="width:120%">
				<li id="tab_postedit" class="active"><a><span>修改文章</span></a></li>
		...
		<div id="moreDiv">
			...
			<div class="radioBox channel" style="margin-top:4px">				
				<c:forEach var="tag" items="${tags}">	
					<c:if test="${blog.type==tag.id}">			
						<input type="radio" name="type"  checked="checked" value="${tag.id}"><label>${tag.name}</label>
					</c:if>
					<c:if test="${blog.type!=tag.id}">			
		...
		<div>
			<input type="submit" value="修改文章" title="保存并跳转">
		</div>
